	...
	<div class="container">
		<?php
			$server_x_data = [
				...
				"altDashboard" => $_SESSION["ALT_DASHBOARD"] === "true",
			];
		?>
			<form
			...
			<details class="box-collapse u-mb10">
				...
				<div class="box-collapse-content">
					...
					<?php
						...
						$available_dashboard_themes = $theme_manager->getAvailableThemes();
						if (!in_array("original", $available_dashboard_themes, true)) {
							array_unshift($available_dashboard_themes, "original");
						}
						$current_color_theme = $theme_manager->getCurrentCssTheme();
						$filtered_color_themes = $theme_manager->getFilteredCssThemes($current_dashboard_theme);
					?>
					<div class="form-check u-mb20">
						<input
							class="form-check-input"
							type="checkbox"
							name="v_alt_dashboard"
							id="v_alt_dashboard"
							x-model="altDashboard"
							<?= tohtml($_SESSION["ALT_DASHBOARD"] == "true" ? "checked" : "") ?>
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= tohtml($_SESSION["token"]) ?>&action=alt_dashboard&enabled=' + ($event.target.checked ? 'true' : 'false')
							})"
						>
						<label for="v_alt_dashboard">
							<?= tohtml( _("Alt Dashboard")) ?>
						</label>
						<span class="hint"><?= tohtml( _("Use the redesigned dashboard as your landing page")) ?></span>
					</div>
					<div class="u-mb10" x-show="altDashboard" x-cloak>
						<label for="v_dashboard_theme" class="form-label">
							<?= tohtml( _("Dashboard Theme")) ?>
						</label>
						<select
							class="form-select"
							id="v_dashboard_theme"
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= tohtml($_SESSION["token"]) ?>&action=dashboard_theme&theme=' + $event.target.value
							}).then(() => location.reload())"
						>
							<?php foreach ($available_dashboard_themes as $dt) { ?>
								<option value="<?= tohtml($dt) ?>" <?= $dt === $current_dashboard_theme ? "selected" : "" ?>>
									<?= tohtml(ucwords(str_replace(["-", "_"], " ", $dt))) ?>
								</option>
							<?php } ?>
						</select>
					</div>
					<div class="u-mb10" x-show="altDashboard" x-cloak>
						<label for="v_color_theme" class="form-label">
							<?= tohtml( _("Color Theme")) ?>
						</label>
						<select
							class="form-select"
							id="v_color_theme"
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= tohtml($_SESSION["token"]) ?>&action=css_theme&theme=' + $event.target.value
							}).then(() => location.reload())"
						>
							<?php foreach ($filtered_color_themes as $ct) { ?>
								<option value="<?= tohtml($ct) ?>" <?= $ct === $current_color_theme ? "selected" : "" ?>>
									<?= tohtml(ucwords(str_replace(["-", "_"], " ", $ct))) ?>
								</option>
							<?php } ?>
						</select>
						<span class="hint"><?= tohtml( _("Only color themes for the selected Dashboard Theme are shown")) ?></span>
					</div>
					<div class="form-check u-mb20">
						<input
							class="form-check-input"
