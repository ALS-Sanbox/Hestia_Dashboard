...
class HestiaThemeManager {
	...
	/**
	 * Get theme metadata from theme.json - UPDATED with themes whitelist
	 */
	public function getThemeMetadata($theme_name) {
		...
		$default_info = [
			...
			'author' => 'Unknown',
			'css_theme' => 'default',
			'themes' => [], // ADDED: CSS themes whitelist
	...
	/**
	 * Install theme CSS files - RESTORED to original behavior
	 */
	...
	/**
	 * Remove theme CSS files - RESTORED to original behavior
	 */
    ...
    /**
     * Set CSS theme for Hestia Control Panel - UPDATED with validation
     */
    public function setCssTheme($css_theme, $validate_against_dashboard = false) {
        try {
            // Optional validation against current dashboard theme
            if ($validate_against_dashboard) {
                $current_dashboard = $this->getCurrentTheme();
                if (!$this->isValidCssThemeForDashboard($css_theme, $current_dashboard)) {
                    throw new Exception("CSS theme '$css_theme' is not allowed for dashboard theme '$current_dashboard'");
                }
            }
    ...
    /**
     * Get available CSS themes - UPDATED with optional filtering
     */
    public function getAvailableCssThemes($filter_by_theme = null) {
        // If filtering requested, get whitelist from theme
        if ($filter_by_theme !== null) {
            return $this->getFilteredCssThemes($filter_by_theme);
        }
        
        // Otherwise return all available CSS themes (existing behavior)
        $css_themes = ['default']; // RESTORED default
    ...
    /**
     * Get filtered CSS themes based on dashboard theme whitelist - RESTORED
     */
    public function getFilteredCssThemes($dashboard_theme) {
        $this->log("Getting filtered CSS themes for dashboard theme: $dashboard_theme");
        
        // Get theme metadata
        $metadata = $this->getThemeMetadata($dashboard_theme);
        
        // Get whitelist from theme.json
        $whitelist = $metadata['themes'] ?? [];
        
        $this->log("Theme whitelist: " . json_encode($whitelist));
        
        // If no whitelist defined, return all themes
        if (empty($whitelist)) {
            $this->log("No whitelist defined, returning all CSS themes");
            return $this->getAvailableCssThemes();
        }
        
        // Get all available CSS themes
        $all_css_themes = $this->getAvailableCssThemes();
        
        // Filter based on whitelist
        $filtered_themes = array_intersect($all_css_themes, $whitelist);
        
        // Always include 'default' as fallback - RESTORED
        if (!in_array('default', $filtered_themes)) {
            array_unshift($filtered_themes, 'default');
        }
        
        // Re-index array
        $filtered_themes = array_values($filtered_themes);
        
        $this->log("Filtered CSS themes: " . json_encode($filtered_themes));
        
        return $filtered_themes;
    }
    
    /**
     * Validate CSS theme against current dashboard theme - NEW
     */
    public function isValidCssThemeForDashboard($css_theme, $dashboard_theme) {
        $allowed_themes = $this->getFilteredCssThemes($dashboard_theme);
        return in_array($css_theme, $allowed_themes);
    }
...
if (php_sapi_name() === 'cli') {
    ...
    if (isset($argv[1])) {
			...
			if (empty($themes)) {
				...
				foreach ($themes as $theme_name => $info) {
					...
					if (!empty($info['themes'])) {
						echo "    Allowed CSS Themes: " . implode(', ', $info['themes']) . "\n";
					}
					echo "\n";
				}
			}
			...
			if (isset($argv[2])) {
				...
				if ($theme_name === 'original') {
					...
					if (!empty($info['themes'])) {
						...
						echo "Allowed CSS Themes: All (no whitelist)\n";
					}
		...
		case 'validate-theme':
			if (isset($argv[2])) {
				...
				if ($theme_name === 'original') {
					...
					foreach ($required_items as $item => $type) {
						...
						if (file_exists($path)) {
							...
							if ($actual_type === $type) {
								...
								echo "[FAIL] $item (expected $type, found $actual_type)\n";
								$valid = false;
							}
						} else {
					...
					if (file_exists($theme_json)) {
						...
						$json_valid = $json_data !== null;
						echo ($json_valid ? "[OK]" : "[WARN]") . " theme.json (" . ($json_valid ? "valid" : "invalid JSON") . ")\n";
						
						if ($json_valid && isset($json_data['themes'])) {
							echo "    CSS Theme Whitelist: " . implode(', ', $json_data['themes']) . "\n";
						}
					} else {
						echo "[WARN] theme.json (optional, not found)\n";
					}
					
					// Check for CSS
					$css_file = $theme_dir . '/css/color_theme.css';
					if (file_exists($css_file)) {
						echo "[OK] css/color_theme.css (found)\n";
					} else {
						echo "[WARN] css/color_theme.css (optional, not found)\n";
					}
            ...
            case 'debug':
                // Debug command to help troubleshoot issues
                $status = $theme_manager->getThemeStatus();
                ...
                // Show filtered CSS themes for current dashboard
                $current_dashboard = $theme_manager->getCurrentTheme();
                $filtered_css = $theme_manager->getFilteredCssThemes($current_dashboard);
                echo "\nFiltered CSS Themes for '$current_dashboard':\n";
                foreach ($filtered_css as $css) {
                    echo "  - $css\n";
                }
            ...
            default:
                echo "Hestia Theme Manager v2.1.0 (With CSS Theme Whitelist Support)\n";
        ...
        echo "Hestia Theme Manager v2.1.0 (With CSS Theme Whitelist Support)\n";
        echo "Usage: php hestia_theme_manager.php [command]\n";
        echo "Run with --help or any command for usage information\n";
    }
}
