...
<div class="container">
	<form
		x-data="{
			...
			altDashboard: <?= $_SESSION["ALT_DASHBOARD"] === "true" ? "true" : "false" ?>,
		}"
		id="main-form"
				...
				<div class="box-collapse-content">
					...
					<div class="form-check u-mb20">
						<input
							...
							<?= $_SESSION["ALT_DASHBOARD"] == "true" ? "checked" : "" ?>
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= $_SESSION["token"] ?>&action=alt_dashboard&enabled=' + ($event.target.checked ? 'true' : 'false')
							})"
						>
						<label for="v_alt_dashboard">
							<?= _("Alt Dashboard") ?>
						</label>
						<span class="hint"><?= _("Use the redesigned dashboard as your landing page") ?></span>
					</div>
					...
					<div class="u-mb10" x-show="altDashboard" x-cloak>
						...
						<select
							...
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= $_SESSION["token"] ?>&action=dashboard_theme&theme=' + $event.target.value
							}).then(() => location.reload())"
						>
							<?php foreach ($available_dashboard_themes as $dt) { ?>
								<option value="<?= htmlentities($dt) ?>" <?= $dt === $current_dashboard_theme ? "selected" : "" ?>>
									<?= htmlentities(ucwords(str_replace(["-", "_"], " ", $dt))) ?>
								</option>
							<?php } ?>
						</select>
					</div>

					<!-- Color Theme (filtered to the selected Dashboard Theme) -->
					<div class="u-mb10" x-show="altDashboard" x-cloak>
						...
						<select
							...
							x-on:change="fetch('/list/dashboard/toggle.php', {
								method: 'POST',
								headers: {'Content-Type': 'application/x-www-form-urlencoded'},
								body: 'token=<?= $_SESSION["token"] ?>&action=css_theme&theme=' + $event.target.value
							}).then(() => location.reload())"
						>
							<?php foreach ($filtered_color_themes as $ct) { ?>
								<option value="<?= htmlentities($ct) ?>" <?= $ct === $current_color_theme ? "selected" : "" ?>>
									<?= htmlentities(ucwords(str_replace(["-", "_"], " ", $ct))) ?>
								</option>
							<?php } ?>
						</select>
						<span class="hint"><?= _("Only color themes for the selected Dashboard Theme are shown") ?></span>
					</div>
					<div class="u-mb10">
