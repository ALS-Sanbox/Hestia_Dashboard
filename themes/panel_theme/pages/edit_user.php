			...
			<div class="u-mb10">
				...
				<select
					...
					x-on:change="fetch('/list/dashboard/toggle.php', {
						...
						body: 'token=<?= $_SESSION["token"] ?>&action=user_theme&user=<?= urlencode(trim($v_username, "'")) ?>&theme=' + $event.target.value
					}).then(() => location.reload())"
				>
					<?php foreach ($filtered_color_themes as $value) { ?>
						<option value="<?= htmlentities($value) ?>" <?= $value === $v_user_theme ? "selected" : "" ?>>
							<?= htmlentities(ucwords(str_replace(["-", "_"], " ", $value))) ?>
						</option>
					<?php } ?>
				</select>
				<span class="hint"><?= _("Only color themes for the active Dashboard Theme are shown") ?></span>
			</div>
			<?php } ?>
				<div class="u-mb10">
